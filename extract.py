...
sys.path.append(os.getcwd() + "/pose_est")
from pose_est.lib.preprocess import h36m_coco_format, revise_kpts
from pose_est.lib.hrnet.gen_kpts import gen_video_kpts as hrnet_pose
from pose_est.strided_transformer import Model
from pose_est.camera import *
...
def show2Dpose(kps, img):
    ...
    # 2D skeleton 14 15 16 RIGHT ARM
    connections = [
        [0, 1],     [1, 2],     [2, 3],     [0, 4], 
        [4, 5],     [5, 6],     [0, 7],     [7, 8], 
        [8, 9],     [9, 10],    [8, 11],    [11, 12], 
...
def get_pose3D(video_path, output_dir):
    ...
    args.previous_dir = 'pose_est/checkpoint/pretrained'
    ...
    for i in tqdm(range(video_length)):
        ...
        pose_out_3D = np.append(pose_out_3D, post_out)
    ...
    output_dir_3D_kpts = output_dir + 'input_3D/'
    os.makedirs(output_dir_3D_kpts, exist_ok=True)

    print(pose_out_3D.shape[0])
    frame = int(pose_out_3D.shape[0] / (17 * 3))
    print(frame, type(frame))

    pose_out_3D = pose_out_3D.reshape(frame, 17, 3)
...
if __name__ == "__main__":
    ...
    video_path = './pose_est/video/' + args.video
    video_name = video_path.split('/')[-1].split('.')[0]
    output_dir = './pose_est/output/' + video_name + '/'
